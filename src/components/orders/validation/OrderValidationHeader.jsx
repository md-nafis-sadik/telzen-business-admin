...
function OrderValidationHeader() {
  const {
    orders,
    validationSearch,
    validationFilter,
  ...
  const { divisions, districts, upazilas } = useLocation(
    validationFilter?.division,
    validationFilter?.district
  );

  // Convert bn_name to ID for LocationSelect value
  const divisionId = divisions.find(d => d.bn_name === validationFilter?.division)?.id || validationFilter?.division || "all";
  const districtId = districts.find(d => d.bn_name === validationFilter?.district)?.id || validationFilter?.district || "all";
  const upazilaId = upazilas.find(u => u.bn_name === validationFilter?.upazila)?.id || validationFilter?.upazila || "all";
  ...
  return (
    <div className="w-full flex flex-col xl:flex-row items-start md:items-center justify-between gap-4 md:gap-0">
      <h1 className="self-stretch justify-start text-gray-100 text-lg font-semibold leading-relaxed">
        Validation Orders ({total_items || 0})
      </h1>
