...
function OrderPendingHeader() {
  ...
  const { divisions, districts, upazilas } = useLocation(
    pendingFilter?.division,
    pendingFilter?.district
  );

  // Convert bn_name to ID for LocationSelect value
  const divisionId = divisions.find(d => d.bn_name === pendingFilter?.division)?.id || pendingFilter?.division || "all";
  const districtId = districts.find(d => d.bn_name === pendingFilter?.district)?.id || pendingFilter?.district || "all";
  const upazilaId = upazilas.find(u => u.bn_name === pendingFilter?.upazila)?.id || pendingFilter?.upazila || "all";
...
export default OrderPendingHeader;
