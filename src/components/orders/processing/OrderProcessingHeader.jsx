...
function OrderProcessingHeader() {
  const {
    orders,
    processingSearch,
    processingFilter,
  ...
  const { divisions, districts, upazilas } = useLocation(
    processingFilter?.division,
    processingFilter?.district
  );

  // Convert bn_name to ID for LocationSelect value
  const divisionId = divisions.find(d => d.bn_name === processingFilter?.division)?.id || processingFilter?.division || "all";
  const districtId = districts.find(d => d.bn_name === processingFilter?.district)?.id || processingFilter?.district || "all";
  const upazilaId = upazilas.find(u => u.bn_name === processingFilter?.upazila)?.id || processingFilter?.upazila || "all";
  ...
  return (
    <div className="w-full flex flex-col xl:flex-row items-start md:items-center justify-between gap-4 md:gap-0">
      <h1 className="self-stretch justify-start text-gray-100 text-lg font-semibold leading-relaxed">
        Processing Orders ({total_items || 0})
      </h1>
