...
function CustomerPendingHeader() {
  const {
    customers,
    activeSearch,
    pendingFilter,
  ...
  const divisionId =
    divisions.find((d) => d.bn_name === pendingFilter?.division)?.id ||
    pendingFilter?.division ||
    "all";
  const districtId =
    districts.find((d) => d.bn_name === pendingFilter?.district)?.id ||
    pendingFilter?.district ||
    pendingFilter;
  ("all");
  const upazilaId =
    upazilas.find((u) => u.bn_name === pendingFilter?.upazila)?.id ||
    pendingFilter?.upazila ||
    "all";
  ...
  return (
    <div className="w-full flex flex-col xl:flex-row items-start md:items-center justify-between gap-4 md:gap-0">
      <h1 className="self-stretch justify-start text-gray-100 text-lg font-semibold leading-relaxed">
        Pending Customers ({total_items || 0})
      </h1>
...
export default CustomerPendingHeader;
